 ...
 * Wyciągnięte z `src/app/page.tsx`, żeby edytowalny canvas i widok domyślny
 * czytały identyczne liczby (react-query deduplikuje fetch).
 */
...
function fxDelta(data: unknown): number | null {
    ...
    return a && b ? +((a - b) / b * 100).toFixed(2) : null;
}
...
export function useOverviewData() {
    ...
    const wigDelta = wigBars.length > 1 ? +((wigBars[wigBars.length - 1].close - wigBars[wigBars.length - 2].close) / wigBars[wigBars.length - 2].close * 100).toFixed(2) : null;
    const goldLast = lastOf(gold), goldDelta = gold.length > 1 ? +((gold[gold.length - 1].value - gold[gold.length - 2].value) / gold[gold.length - 2].value * 100).toFixed(2) : null;

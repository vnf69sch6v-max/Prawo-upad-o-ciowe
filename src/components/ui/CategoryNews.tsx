...
export function CategoryNews({
    ...
    const items = useMemo(() => {
        ...
        return matched
            ...
            .slice(0, limit);
    }, [data, topic, limit, excludeDateKeys]);
...
export function Kalendarium({
    ...
    const news = useMemo(
        () => matchNews(newsData?.items ?? [], topic, newsLimit),
        [newsData, topic, newsLimit],
    );
...
export function CategoryNewsPanel({
    ...
    const showKalendarium = useMemo(() => {
        const news = matchNews(newsData?.items ?? [], topic, 20);
    ...
    }, [newsData, digestToday, digestYesterday, topic, todayKey, yesterdayKey]);

    if (!showKalendarium) {
        return <CategoryNews topic={topic} limit={limit} className={className} />;
    }

    const newsOnly = (
        <CategoryNews topic={topic} limit={limit} excludeDateKeys={recentKeys} />
    );

    return (
        <div className={`space-y-4 ${className}`.trim()}>
            <Kalendarium topic={topic} newsLimit={limit + 7} />
            {newsOnly}
        </div>
    );
}

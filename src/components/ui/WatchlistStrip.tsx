 ...
 */
export function WatchlistStrip({ items }: { items: WatchableKpi[] }) {
    ...
    return (
        <section aria-label="Obserwowane wskaźniki">
            <h2 className="mk-section-label mb-3 flex items-center gap-2">
                <Star size={13} className="fill-mk-brand text-mk-brand" aria-hidden />
                Obserwowane
            </h2>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
                {watched.map((k) => (
                    <KpiCard key={k.watchId} {...k} watchId={k.watchId} />
                ))}
            </div>
        </section>
    );

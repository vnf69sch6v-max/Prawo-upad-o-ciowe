...
type Section = 'inflacja' | 'aktywnosc' | 'praca' | 'stopy';
    ...
    return (
        <div className="mk-fade-in space-y-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-mk-text">Dane makro</h1>
                    <p className="mt-1 text-sm text-mk-muted">Inflacja, aktywność gospodarcza, rynek pracy i stopy procentowe</p>
                </div>
                <Segmented value={section} onChange={setSection} options={SECTIONS} aria-label="Sekcja" />
            </div>

            {section === 'inflacja' && <InflacjaSection />}
            {section === 'aktywnosc' && <AktywnoscSection />}
            {section === 'praca' && <RynekPracySection />}

...
import { TrendingUp, Percent, Users, BarChart3, Factory, Euro, DollarSign, ShoppingCart, LineChart, Landmark, Gem, Building2 } from 'lucide-react';
import {
    useCpiFull, useUnemploymentMonthly, useGDPQuarterly,
    useNBPInterestRates, useNBPTable, useEURPLN, useUSDPLN,
    useIndustrialProduction, useRetailSales, useBondYield10Y, useGold, useStooq, useWig20,
...
export default function OverviewPage() {
    ...
    const watchesCompany = watchlist.items.some((i) => i.kind === 'spolka');
    const spolkiQ = useWig20(watchesCompany);
    ...
    // ── KPI makro ──
    // `watch.id` to STABILNY slug, nie etykieta — etykieta bywa przeredagowana („Stopa bezrobocia"
    // → „Bezrobocie rejestrowane"), a to skasowałoby użytkownikowi watchlistę w localStorage.
    const macro = [
        { label: 'Inflacja CPI (r/r)', href: '/ceny?tab=inflacja', value: fmt1(lastOf(cpi)), unit: '%', accent: 'amber' as AccentKey, icon: TrendingUp, delta: ppDelta(cpi) != null ? { value: ppDelta(cpi)!, unit: 'pp' as const, invert: true } : undefined, footnote: 'GUS · cel NBP 2,5%', loading: cpiQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'cpi' } },
        { label: 'PKB (r/r)', href: '/gospodarka?tab=aktywnosc', value: fmt1(lastOf(gdp)), unit: '%', accent: 'green' as AccentKey, icon: BarChart3, delta: ppDelta(gdp) != null ? { value: ppDelta(gdp)!, unit: 'pp' as const } : undefined, footnote: gdp.length ? `Eurostat · ${gdp[gdp.length - 1].date}` : 'Eurostat', loading: gdpQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'pkb' } },
        { label: 'Stopa bezrobocia', href: '/praca?tab=bezrobocie', value: fmt1(lastOf(unemp)), unit: '%', accent: 'blue' as AccentKey, icon: Users, delta: ppDelta(unemp) != null ? { value: ppDelta(unemp)!, unit: 'pp' as const, invert: true } : undefined, footnote: 'Eurostat LFS', loading: unempQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'bezrobocie' } },
        { label: 'Stopa referencyjna NBP', href: '/rynki?tab=stopy', value: refRate ? formatDecimalPL(refRate.value, 2) : '—', unit: '%', accent: 'violet' as AccentKey, icon: Percent, footnote: refRate ? `NBP · od ${formatDate(refRate.validFrom)}` : 'NBP', loading: ratesQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'stopa-ref' } },
        { label: 'Produkcja przemysłowa (r/r)', href: '/gospodarka?tab=aktywnosc', value: fmt1(lastOf(industrial)), unit: '%', accent: 'rose' as AccentKey, icon: Factory, delta: ppDelta(industrial) != null ? { value: ppDelta(industrial)!, unit: 'pp' as const } : undefined, footnote: industrial.length ? `Eurostat · ${industrial[industrial.length - 1].date}` : 'Eurostat', loading: indQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'produkcja' } },
        { label: 'Sprzedaż detaliczna (r/r)', href: '/gospodarka?tab=aktywnosc', value: fmt1(lastOf(retail)), unit: '%', accent: 'cyan' as AccentKey, icon: ShoppingCart, delta: ppDelta(retail) != null ? { value: ppDelta(retail)!, unit: 'pp' as const } : undefined, footnote: retail.length ? `Eurostat · ${retail[retail.length - 1].date}` : 'Eurostat', loading: retailQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'sprzedaz' } },
    ];

    // ── KPI rynki ──
    const markets = [
        { label: 'WIG20', href: '/rynki?tab=gpw', value: wigLast != null ? formatNumber(wigLast, 0) : '—', unit: 'pkt', accent: 'blue' as AccentKey, icon: LineChart, delta: wigDelta != null ? { value: wigDelta, unit: 'pct' as const } : undefined, footnote: 'GPW · Stooq/Yahoo', loading: wig20Q.isLoading, watch: { kind: 'wskaznik' as const, id: 'wig20' } },
        { label: 'EUR / PLN', href: '/rynki?tab=kursy', value: mid('EUR') != null ? formatDecimalPL(mid('EUR')!, 3) : '—', unit: 'zł', accent: 'cyan' as AccentKey, icon: Euro, delta: fxDelta(eurHQ.data) != null ? { value: fxDelta(eurHQ.data)!, unit: 'pct' as const, invert: true } : undefined, footnote: fxTable?.effectiveDate ? `NBP ${formatDate(fxTable.effectiveDate)}` : 'NBP', loading: fxQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'eurpln' } },
        { label: 'USD / PLN', href: '/rynki?tab=kursy', value: mid('USD') != null ? formatDecimalPL(mid('USD')!, 3) : '—', unit: 'zł', accent: 'green' as AccentKey, icon: DollarSign, delta: fxDelta(usdHQ.data) != null ? { value: fxDelta(usdHQ.data)!, unit: 'pct' as const, invert: true } : undefined, footnote: fxTable?.effectiveDate ? `NBP ${formatDate(fxTable.effectiveDate)}` : 'NBP', loading: fxQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'usdpln' } },
        { label: 'Rentowność 10Y', href: '/gospodarka?tab=finanse', value: lastOf(yield10) != null ? formatDecimalPL(lastOf(yield10)!, 2) : '—', unit: '%', accent: 'violet' as AccentKey, icon: Landmark, delta: lastOf(yield10) != null && prevOf(yield10) != null ? { value: +(lastOf(yield10)! - prevOf(yield10)!).toFixed(2), unit: 'pp' as const, invert: true } : undefined, footnote: yield10.length ? `Eurostat · ${yield10[yield10.length - 1].date}` : 'Eurostat', loading: yieldQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'obligacje-10y' } },
        { label: 'Złoto (NBP)', href: '/rynki?tab=kursy', value: goldLast != null ? formatDecimalPL(goldLast, 2) : '—', unit: 'zł/g', accent: 'amber' as AccentKey, icon: Gem, delta: goldDelta != null ? { value: goldDelta, unit: 'pct' as const } : undefined, footnote: 'NBP · cena złota', loading: goldQ.isLoading, watch: { kind: 'wskaznik' as const, id: 'zloto' } },
    ];

    // ── pas „Obserwowane" ──────────────────────────────────
    // Wskaźniki bierzemy z TYCH SAMYCH tablic co niżej — zero dodatkowych żądań, kafel jest
    // dokładnie tym samym obiektem propsów. Spółki wymagają `/api/wig20`, więc pytamy o nie
    // dopiero wtedy, gdy użytkownik faktycznie którąś obserwuje.
    const watchedKpis = [...macro, ...markets].filter((k) => watchlist.has('wskaznik', k.watch.id));
    const watchedCompanies = WIG20.filter((c) => watchlist.has('spolka', c.ticker));
    // Przy hydracji lista jest jeszcze pusta (snapshot serwerowy), więc pas pojawia się dopiero
    // w kolejnym renderze — i właśnie dzięki temu nie ma niezgodności HTML serwer/klient.
    const hasWatched = watchedKpis.length > 0 || watchedCompanies.length > 0;
    ...
    return (
        <div className="mk-fade-in space-y-6">
            ...
            {/* Obserwowane — pas pojawia się dopiero, gdy użytkownik coś oznaczył gwiazdką.
                Pusta ramka z zachętą byłaby szumem na stronie, którą większość ogląda bez watchlisty;
                gwiazdki przy kaflach niżej same tłumaczą, jak ten pas zapełnić. */}
            {hasWatched && (
                <SectionCard
                    title="Obserwowane"
                    subtitle="Twój wybór — zapisany w tej przeglądarce, na tym urządzeniu"
                    actions={
                        <button type="button" onClick={watchlist.clear}
                            className="rounded-lg px-2 py-1 text-xs font-medium text-mk-muted transition-colors hover:bg-mk-surface-alt hover:text-mk-text">
                            Wyczyść
                        </button>
                    }>
                    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4 xl:grid-cols-5">
                        {watchedKpis.map((k) => <KpiCard key={k.watch.id} {...k} />)}
                        {watchedCompanies.map((c) => {
                            const q = spolkiQ.data?.items.find((x) => x.ticker === c.ticker) ?? null;
                            return (
                                <KpiCard
                                    key={c.ticker}
                                    label={c.name}
                                    ...
                                    unit="zł"
                                    delta={q?.changePct != null ? { value: q.changePct, unit: 'pct' } : undefined}
                                    footnote={q?.date ? `${c.ticker} · GPW · ${q.date}` : `${c.ticker} · GPW`}
                                    loading={spolkiQ.isLoading}
                                    watch={{ kind: 'spolka', id: c.ticker }}
                                />
                            );
                        })}
                    </div>
